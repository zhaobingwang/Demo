...
namespace TopshelfDemo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                x.Service<Operation>(s =>
                {
                    s.ConstructUsing(name => new Operation());
                    s.WhenStarted(foo => foo.ExcuteJob());
    ...
    public class TimeJob : IJob
    {
        public Task Execute(IJobExecutionContext context)
        {
            Task t = Task.Factory.StartNew(() =>
            {
                ...
                System.IO.File.AppendAllText(@"E:\temp.txt", DateTime.Now + Environment.NewLine);
            });
            return t;
        }
    }
